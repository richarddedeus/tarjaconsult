...
class Upload {
    ...
    public function Image(array $Image, $Name = null, $Width = null, $Folder = null) {
        ...
        $this->Folder = ( (string) $Folder ? $Folder : 'img' );
    ...
    public function File(array $File, $Name = null, $Folder = null, $MaxFileSize = null) {
        ...
        if ($this->File['size'] > ($MaxFileSize * (1024 * 1024))):
            $this->Result = false;
            $this->Error = "Execedido tamanho máximo do arquivo. Tamanho permitido: {$MaxFileSize}mb";
        elseif (!in_array($this->File['type'], $FileAccept)):
            $this->Result = false;
            $this->Error = 'Tipo de arquivo não suportado. Envie .PDF, .TXT ou .DOCX/.DOC';
        else:
            $this->CheckFolder($this->Folder);
            $this->setFileName();
            $this->MoveFile();
    ...
    //Realiza o upload de imagens redimensionando-o
        ...
        switch ($this->File['type']):
            ...
            case 'image/x-png':
                ...
                case 'image/x-windows-bmp':
                    $this->Image = imagecreatefrombmp($this->File['tmp_name']);
                    break;
        endswitch;

        if (!$this->Image):
            $this->Result = false;
            $this->Error = 'Tipo de arquivo inválido, arquivos permitidos: JPG,PNG, GIF OU BMP.';
        else:
            ...
            switch ($this->File['type']):
                ...
                case 'image/bmp':
                case 'image/x-windows-bmp':
                    imagebmp($NewImage, self::$BaseDir . $this->Send . $this->Name);
                    break;
            endswitch;
